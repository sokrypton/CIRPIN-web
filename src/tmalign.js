...
export function kabsch(x, y, n, mode, t, u, out) {
  const tol = 0.01;
  const epsilon = 1e-8;
...
export function nwdpRot(path, val, x, y, len1, len2, t, u, d02, gapOpen, j2i) {
  const w = len2 + 1;
  let prev = val.subarray(0, w);
  let cur = val.subarray(w, 2 * w);
  nwInit(path, prev, len1, len2, j2i, w);
...
export function nwdpSec(path, val, secx, secy, len1, len2, gapOpen, j2i) {
  const w = len2 + 1;
  let prev = val.subarray(0, w);
  let cur = val.subarray(w, 2 * w);
  nwInit(path, prev, len1, len2, j2i, w);
...
export function getScoreFast(W, x, y, ylen, invmap, d0, d0Search, t, u) {
...
export function getInitialFgt(W, x, y, xlen, ylen, y2x, d0, d0Search, dcu0, fastOpt, t, u) {
  let fraMin = 4;
  if (fastOpt) fraMin = 8;
...
export function makeWorkspace(xlen, ylen) {
  const minlen = Math.min(xlen, ylen);
  return {
 ...
 */
export function tmalignMain(xa, ya, secx, secy, xlen, ylen, fastOpt) {
 ...
 */
export function cpAlign(xa, ya, xlen, ylen, opts = {}) {
  ...
  /* pass 1: align the duplicated chain (always fast, as in the C++) */
  const dup = tmalignMain(xaCp, ya, secxCp, secy, xlen * 2, ylen, true);
  ...
  /* pass 2: plain sequential alignment, to decide whether the CP is real */
  const lin = tmalignMain(xa, ya, secx, secy, xlen, ylen, true);
...
export function permuteCoords(xa, n, k) {
...
export function tmAlign(xa, ya, xlen, ylen, opts = {}) {
...
export function applyTransform(xa, n, t, u) {
  const out = new Float64Array(n * 3);
 ...
 * inversion involved.
 */
export function applyInverseTransform(ya, n, t, u) {
  const out = new Float64Array(n * 3);
  for (let i = 0; i < n; i++) {
    const a = ya[i * 3] - t[0];
